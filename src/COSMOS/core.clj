...
(defn cosmos-1
  ...
  ([data-seq full-stream?]
     (loop [recs (sorted-map) data data-seq converged #{} unconverged #{}]
       (cond (not (seq data)) (reset! last-computed (with-meta recs {:converged converged :unconverged unconverged}))
	     (and (not full-stream?) (seq converged) (not (seq unconverged))) (reset! last-computed (with-meta recs {:converged converged :unconverged unconverged}))
	     :else (let [[{ord :ord val :val gen :gen} & tail] data]
		     (if (contains? converged [gen ord]) 
		       ...
		       (let [{runs :runs xbar :xbar} (-> recs (get gen {}) (get ord {:runs 0 :xbar 1}))
			     new-xbar (/ (+ (* xbar runs) val) (inc runs))]
...
(defn- get-converged-data
  ...
  (let [[gen ord] (first unconverged)]
    (if (empty? unconverged) cd (recur (rest unconverged) (assoc cd gen (dissoc (get cd gen) ord))))))
